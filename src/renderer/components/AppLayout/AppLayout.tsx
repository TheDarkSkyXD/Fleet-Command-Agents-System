...
function parseHash(hash: string): { page: string; agentId?: string } {
  ...
  if (!cleaned) return { page: 'command-center' };
...
function buildHash(page: string, agentId?: string | null): string {
  ...
  if (page === 'command-center' && !agentId) return '';
  return `#/${page}`;
}
...
export function AppLayout() {
  ...
  // Event-driven agent count for window title (no polling)
  useEffect(() => {
  ...
  const handleConfirmLeave = useCallback(() => {
    ...
    if (nav) {
      executeNavigation(nav.page || 'command-center', nav.agentId);
    }
  ...
  // No longer needed - projects tab handles this directly
  ...
  // Setup wizard: check persistent electron-store (survives DB resets)
  const [setupCompleted, setSetupCompleted] = useState(true); // default true to avoid flash
  useEffect(() => {
    window.electronAPI.storeGet('setupCompleted').then((val) => {
      setSetupCompleted(val === true);
    });
  }, []);
  const showSetupWizard = loaded && !setupCompleted;

  const handleSetupComplete = useCallback(() => {
    setSetupCompleted(true);
    window.electronAPI.storeSet('setupCompleted', true);
    ...
    >
      ...
      <div className="flex flex-1 overflow-hidden" data-testid="app-main-area">
        ...
        <ErrorBoundary sectionName="Sidebar">
          <Sidebar
            currentPage={currentPage === 'agent-detail' ? 'command-center' : currentPage}
            ...
            >
              <ErrorBoundary sectionName="Page content">
                <PageContent
                  ...
                  onNavigateHome={() => handleNavigate('command-center')}
                />
              </ErrorBoundary>
            </motion.div>
          </AnimatePresence>
      ...
      {currentPage === 'command-center' && <OnboardingTour />}
    ...
    case 'command-center':
      return (
        <ErrorBoundary sectionName="Command Center">
          <CommandCenterPage onSelectAgent={onSelectAgent} />
        </ErrorBoundary>
      );
    case 'agents':
      return (
