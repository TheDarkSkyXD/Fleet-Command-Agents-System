...
export function StatusBar({ onNavigate }: StatusBarProps) {
  ...
  // Event-driven active run updates (no polling)
  useEffect(() => {
    fetchActiveRun();

    // Refresh when agent state changes (stop/spawn) — no interval needed
  ...
  useEffect(() => {
    ...
    // Re-fetch progress on agent state changes
    const unsubAgentUpdate = window.electronAPI.onAgentUpdate(() => {
      fetchProgress();
    });
    return () => { unsubAgentUpdate(); };
  ...
  // Load profiles once on mount (no polling — profiles rarely change)
  useEffect(() => {
    loadProfiles();
  }, [loadProfiles]);
        ...
        {allProfiles.length > 0 && (
          <div className="relative" ref={profileMenuRef}>
            <Button
              ...
              className="flex items-center gap-1 h-auto px-1.5 py-0.5"
              ...
              <div className="absolute bottom-full right-0 mb-1 w-56 rounded-lg border border-slate-700 bg-slate-800 shadow-xl z-50 py-1 max-h-64 overflow-y-auto">
                ...
                {allProfiles.map((p) => (
                  <Button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-1.5 text-xs text-left hover:bg-slate-700 transition-colors h-auto justify-start rounded-none ${
        ...
        {activeRun ? (
          <span className="flex items-center gap-2" data-testid="status-bar-run">
            ...
            <Button
              ...
              className="flex items-center gap-1 rounded px-1.5 py-0.5 text-emerald-400 hover:bg-emerald-500/20 hover:text-emerald-300 h-auto"
            ...
            <Button
              ...
              className="flex items-center gap-1 rounded px-1.5 py-0.5 text-red-400 hover:bg-red-500/20 hover:text-red-300 h-auto"
              title="Stop run"
            >
